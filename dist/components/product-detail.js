...
class ProductDetailPage {
    ...
    loadProduct() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                this.product = Object.assign(Object.assign({}, product), { title: product.title || 'Unknown Product', description: product.description || 'No description available', category: product.category || 'Uncategorized', price: typeof product.price === 'number' ? product.price : 0, discountPercentage: typeof product.discountPercentage === 'number' ? product.discountPercentage : 0, rating: typeof product.rating === 'number' ? product.rating : 0, stock: typeof product.stock === 'number' ? product.stock : 0, tags: Array.isArray(product.tags) ? product.tags : [], brand: product.brand || 'Unknown Brand', sku: product.sku || 'UNKNOWN-SKU', weight: typeof product.weight === 'number' ? product.weight : 0, dimensions: product.dimensions && typeof product.dimensions === 'object'
                        ? {
                        ...
                        : { width: 0, height: 0, depth: 0 }, warrantyInformation: product.warrantyInformation || 'No warranty information', shippingInformation: product.shippingInformation || 'No shipping information', availabilityStatus: product.availabilityStatus || 'Unknown', reviews: Array.isArray(product.reviews) ? product.reviews : [], returnPolicy: product.returnPolicy || 'No return policy', minimumOrderQuantity: typeof product.minimumOrderQuantity === 'number' ? product.minimumOrderQuantity : 1, meta: product.meta && typeof product.meta === 'object'
                        ? {
                        ...
                        }, images: Array.isArray(product.images) ? product.images : [], thumbnail: product.thumbnail || 'https://via.placeholder.com/300' });
            }
            catch (error) {
    ...
    createStarElement(filled = true, size = 'w-5 h-5') {
    ...
    createStarsContainer(rating, size = 'w-5 h-5') {
    ...
    increaseQuantity() {
        if (this.product && this.quantity < this.product.stock) {
            this.quantity++;
            document.getElementById('quantity-display').textContent = this.quantity.toString();
    ...
    createTabsSection() {
        ...
        contentContainer.append(this.createShippingContent(), this.createReviewsContent(), this.createSpecsContent());
        container.appendChild(contentContainer);
        return container;
    }
    createShippingContent() {
        const container = this.createElement('div', {
            id: 'content-shipping',
    ...
    render() {
        ...
        this.container.append(this.createBackNavigation(), mainContainer, this.createTabsSection());
